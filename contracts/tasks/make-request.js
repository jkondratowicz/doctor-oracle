...
task('make-request', 'Initiates an on-demand request from a Functions consumer contract')
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    await utils.prompt(
      `If the request's callback uses all ${utils.numberWithCommas(callbackGasLimit)} gas, this request will charge the subscription an estimated ${chalk.blue(
    ...
    try {
      ...
      switch (fulfillmentCode) {
        ...
        case FulfillmentCode.USER_CALLBACK_ERROR:
          spinner.fail(
            'Error encountered when calling consumer contract callback.\nEnsure the fulfillRequest function in DoctorOracle is correct and the --callbackgaslimit is sufficient.'
          );
          break;
